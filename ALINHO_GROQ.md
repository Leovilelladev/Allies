...
- Sem troca automática de modelo/provedor, sem repetição automática de erros do provedor e sem ferramentas extras. A cobrança é definida pelo plano da organização Groq: o código não consegue confirmar ou impor o plano Free. `ALINHO_ENABLED` é somente uma chave de ativação.
- Até 1.200 caracteres por pergunta; até 500 tokens para identificar os termos de busca em inglês e 1.200 para explicar os trechos (incluindo raciocínio), com prazo total de 20 segundos. Duas chamadas à Groq por pergunta respondida: termos de busca e resposta. Busca vazia usa uma chamada. Se a resposta omitir citações ou usar números inválidos, há uma única revisão com os mesmos trechos (até três chamadas no total), dentro do mesmo prazo de 20 segundos e sujeita à cota gratuita. São enviados a pergunta, instruções fixas e até quatro trechos de 2.200 caracteres do SRD. Sem histórico, rascunho, chave do Supabase ou dados de campanhas.
...
O modelo recebe somente os trechos recuperados e deve citar seus números. O servidor solicita uma única revisão de respostas com referências fora da lista ou sem nenhuma referência quando há trechos. Se a revisão continuar inválida, rejeita a resposta; não insere citações por conta própria. Isso valida a existência da referência, não garante que cada afirmação é fiel: ainda é necessário conferir respostas e extração de tabelas. Busca sem resultados pede esclarecimento, sem gerar regras de memória. O SRD não equivale aos livros comerciais completos; nomes traduzidos podem exigir esclarecimento ou o nome em inglês.
