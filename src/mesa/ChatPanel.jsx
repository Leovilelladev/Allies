  ...
  return (
    <div className="mesa-chat">
      <div className="mesa-chat-lista" ref={listaRef}>
        {mensagens.length === 0 && (
          <div className="mesa-chat-vazio">Nenhuma mensagem ainda. Digite /r 1d20 para rolar.</div>
        )}
        {mensagens.map((m) => (
          <div
            key={m.id}
            className={`mesa-chat-msg ${m.tipo === 'rolagem' ? 'mesa-chat-msg--rolagem' : ''}`}
          >
      ...
      {erro && <div className="mesa-chat-erro">{erro}</div>}
